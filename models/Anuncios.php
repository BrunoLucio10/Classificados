...
class Anuncios extends model {
	...
	public function getUltimosAnuncios($page, $perPage, $filtros) {
		...
		$sql = $this->db->prepare("SELECT
			*,
			(select anuncios_ima.url from anuncios_ima where anuncios_ima.id_anuncio = anuncios.id limit 1) as url,
			(select categoria.nome from categoria where categoria.id = anuncios.id_categoria) as categoria
	...
	public function getAnuncio($id) {
		...
		if($sql->rowCount() > 0) {
			...
			$sql = $this->db->prepare("SELECT id,id_anuncio,url FROM anuncios_ima WHERE id_anuncio = :id_anuncio");
	...
	public function addAnuncio($titulo, $categoria, $valor, $descricao, $estado) {
		...
		$sql = $this->db->prepare("INSERT INTO anuncios SET id_usuario = :id_usuario, id_categoria = :id_categoria, titulo = :titulo, descricao = :descricao, valor = :valor, estado = :estado");
		...
		if(count($fotos) > 0) {
			for($q=0;$q<count($fotos['tmp_name']);$q++) {
				...
				if(in_array($tipo, array('image/jpeg', 'image/png'))) {
					...
					$sql = $this->db->prepare("INSERT INTO anuncios_ima SET id_anuncio = :id_anuncio, url = :url");
					$sql->bindValue(":id_anuncio", $id);
	...
	public function excluirAnuncio($id) {
		...
		$sql = $this->db->prepare("DELETE FROM anuncios_ima WHERE id_anuncio = :id_anuncio");
		$sql->bindValue(":id_anuncio", $id);
		$sql->execute();
	...
	public function excluirFoto($id) {
		...
		$sql = $this->db->prepare("SELECT id_anuncio FROM anuncios_ima WHERE id = :id");
		$sql->bindValue(":id", $id);
		$sql->execute();
		...
		$sql = $this->db->prepare("DELETE FROM anuncios_ima WHERE id = :id");
		$sql->bindValue(":id", $id);
		$sql->execute();
